 ...
 AbstractDistMatrix<T>& CPre)
{
    ...
    DistMatrix<T,STAR,MC  ,ELEMENT,D> B1_STAR_MC(g);
...
{
    LogicError("SUMMA_TTA_impl type-device combo not supported.");
}
        ...
        AbstractDistMatrix<T>& CPre)
{
    ...
    // Temporary distributions
    DistMatrix<T,VR,  STAR,ELEMENT,D> A1_VR_STAR(g);
    DistMatrix<T,STAR,MR  ,ELEMENT,D> A1Trans_STAR_MR(g);
...
(Orientation, Orientation, T,
 ...
 AbstractDistMatrix<T>&)
{
    LogicError("SUMMA_TTB_impl type-device combo not supported.");
}
        ...
        AbstractDistMatrix<T>& CPre)
{
    ...
    for(Int k=0; k<sumDim; k+=bsize)
    {
        ...
        LocalGemm
        (orientA, NORMAL, alpha, A1_STAR_MC, B1Trans_STAR_MR, TypeTraits<T>::One(), C);
    }
}
...
(Orientation, Orientation, T,
 ...
 AbstractDistMatrix<T>&)
{
    LogicError("SUMMA_TTC_impl type-device combo not supported.");
}
...
(Orientation,
 ...
 Int)
{
    LogicError("SUMMA_TTDot_impl type-device combo not supported.");
}
